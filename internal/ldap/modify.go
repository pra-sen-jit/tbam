...
func (c *Client) RevokeSpecificAccess(grant models.AccessGrant) error {
	conn, err := c.Borrow()
	if err != nil {
		return fmt.Errorf("cannot borrow connection: %w", err)
	}
	defer c.Return(conn)
	...
	if err != nil {
		...
		if ok && le.ResultCode == ldap.LDAPResultNoSuchAttribute {
			log.Println("User already not in group, continuing")
		} else {
	...
	// 2. Clear the specific timer attribute
